...
def call_openai(
    ...
    latency = max(float(time.perf_counter() - started), 1e-9)
...
def compare_models(prompt: str) -> dict:
    ...
    full_response, full_latency = call_openai(prompt)
    mini_response, mini_latency = call_openai_mini(prompt)
    estimated_output_tokens = len(full_response.split()) / 0.75
    price = PRICING_PER_1K_TOKENS.get(
        OPENAI_MODEL, PRICING_PER_1K_TOKENS["gpt-4o"]
    )
    return {
        "gpt4o_response": full_response,
        "mini_response": mini_response,
        "gpt4o_latency": full_latency,
        "mini_latency": mini_latency,
        "gpt4o_cost_estimate": estimated_output_tokens / 1000 * price["output"],
    }
